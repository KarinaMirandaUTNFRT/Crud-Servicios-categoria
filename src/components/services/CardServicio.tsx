      ...
      try {
        window.dispatchEvent(
          new CustomEvent("toast", {
            detail: { message: "Debes iniciar sesión para agregar al carrito" },
          }),
        );
      } catch (e) {
        // fallback
    ...
    try {
      ...
      if (resp.ok) {
        ...
        try {
          window.dispatchEvent(
            new CustomEvent("toast", {
              detail: { message: "Servicio agregado al carrito con éxito" },
            }),
          );
        } catch (e) {
          // fallback
...
{/* Acciones de compra */}
          <div className="flex items-center gap-2">
            <input
              ...
              disabled={loading}
              className="w-16 rounded border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-center text-zinc-100 text-sm focus:outline-none focus:border-blue-500"
            />
            <button
              type="button"
              onClick={handleAgregar}
              disabled={loading}
              className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-1.5 px-3 rounded-lg text-sm font-bold transition-colors shadow-md shadow-blue-900/20 active:scale-95 disabled:opacity-50"
            >
